...
{#if showEventPopup}
  <div class=" fixed inset-0 flex items-center justify-center z-10">
    <div class="bg-white backdrop-blur-md rounded-lg p-2 w-96 shadow-lg">
      ...
      {:else}
        <h3 class="text-xl font-semibold p-2">
          Adding event from {format(startDate, "EEEE 'the' do")} to {format(
            endDate,
            "EEEE 'the' do"
          )}
        </h3>
              ...
              {#each sortedEvents as event, index}
                {#if isWithinInterval( day.date, { start: event.start, end: event.end } )}
                  <div
                    ...
                    class:rounded-l-lg={isSameDay(day.date, event.start)}
                    class:ml-2={isSameDay(day.date, event.start)}
                    class:rounded-r-lg={isSameDay(day.date, event.end)}
                    class:mr-2={isSameDay(day.date, event.end)}
                  >
